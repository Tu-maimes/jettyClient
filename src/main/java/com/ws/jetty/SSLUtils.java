...
public class SSLUtils {
     ...
     */
    public static SslContextFactory createSslContextFactory(HashMap<String, String> sslConfigValues) {

        return createSslContextFactory(sslConfigValues, false);
    }
    ...
    public static SslContextFactory createSslContextFactory(HashMap<String, String> sslConfigValues, boolean client) {
//        Map<String, Object> sslConfigValues = config.valuesWithPrefixAllOrNothing("listeners.https.");
        HashMap<String, Object> hashMap = new HashMap<>();
        for (String key : sslConfigValues.keySet()) {
            if (key.startsWith("listeners.https.")) {
                hashMap.put(key, sslConfigValues.get(key));
            }
        }
        SslContextFactory ssl = new SslContextFactory();

        configureSslContextFactoryKeyStore(ssl, hashMap);
        configureSslContextFactoryTrustStore(ssl, hashMap);
        configureSslContextFactoryAlgorithms(ssl, hashMap);
        configureSslContextFactoryAuthentication(ssl, hashMap);

        if (client)
            configureSslContextFactoryEndpointIdentification(ssl, hashMap);
